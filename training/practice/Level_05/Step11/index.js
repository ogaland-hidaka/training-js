...
// 重複の削除
const uniqueJobObj = Array.from(
    new Map(bravePartyObj.map((character) => [character.job, character])).values()
);
let jobName = '';
uniqueJobObj.forEach((val, index) => {
    jobName += val.job + '、';
});
jobName = jobName.slice(0, -1);
console.log('このパーティー職業の種類は、' + jobName + 'です。');