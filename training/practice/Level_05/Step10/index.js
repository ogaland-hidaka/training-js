...
// ウィザードだけを抽出
const wizardObj = bravePartyObj.filter(party => party['job'] === 'ウィザード');

// 職業がウィザードの名前を取得
let wizardName = '';
wizardObj.forEach((val, index) => {
    wizardName += val.name + '、';
});
wizardName = wizardName.slice(0, -1);
console.log('このパーティーのウィザードたちの名前は、' + wizardName + 'です。');