...
// 重複の削除
const uniqueIntelligenceObj = Array.from(
    new Map(bravePartyObj.map((character) => [character.intelligence, character])).values()
);
let intelligenceVals = '';
uniqueIntelligenceObj.forEach((val, index) => {
    intelligenceVals += val.intelligence + '、';
});
intelligenceVals = intelligenceVals.slice(0, -1);
console.log('このパーティーのちりょくの値は、' + intelligenceVals + 'です。');